...
import { useUploadApplicationMutation } from "../api";
...
export default function UploadApplicationModal({ amendmentId, onClose, onSuccess }: Props) {
  ...
  const [uploadApplication, { isLoading: isUploading }] = useUploadApplicationMutation();

  async function handleUpload() {
    if (!file) return;

    try {
      await uploadApplication({ id: amendmentId, file }).unwrap();
      onSuccess();
    } catch {
      setPrompt({
        title: t("messages.errorTitle") || "Ошибка",
        text: t("jobApplication.messages.uploadError") || "Не удалось загрузить заявление",
        variant: "error",
      });
    }
  }
