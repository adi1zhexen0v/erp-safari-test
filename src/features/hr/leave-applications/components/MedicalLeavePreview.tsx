...
interface Props {
  ...
  formData: CreateLeaveFormValues & { diagnosis?: string };
  onClose: () => void;
  onSubmit: () => void;
  isLoading: boolean;
...
export default function MedicalLeavePreview({
  employee,
  formData,
  onClose,
  onSubmit,
  ...
  const diagnosis = formData.diagnosis || "_____________";
        ...
        <div className="flex-1 overflow-auto page-scroll pr-5 pt-5">
          <div className="flex flex-col text-body-regular-md content-base-primary bg-white">
            ...
            <div className="mb-6 text-justify">
              <p>
                Прошу предоставить мне больничный лист в количестве <strong>{daysCount}</strong> календарных дней с{" "}
                <strong>{startDateText}</strong> по <strong>{endDateText}</strong>.
              </p>
              {diagnosis !== "_____________" && (
                <p className="mt-4">
                  Диагноз: <strong>{diagnosis}</strong>
                </p>
              )}
            </div>
