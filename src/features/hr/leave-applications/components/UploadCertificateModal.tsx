...
import { useUploadMedicalCertificateMutation } from "../api";
...
export default function UploadCertificateModal({ leaveId, onClose, onSuccess }: Props) {
  ...
  const [uploadCertificate, { isLoading: isUploading }] = useUploadMedicalCertificateMutation();

  async function handleUpload() {
    if (!file) return;

    try {
      await uploadCertificate({ id: leaveId, file }).unwrap();
      onSuccess();
    } catch {
      setPrompt({
        title: t("messages.errorTitle") || "Ошибка",
        text: t("messages.uploadCertificateError") || "Не удалось загрузить справку",
        variant: "error",
      });
    }
  }
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2 border-b surface-base-stroke pb-3">
            <h4 className="text-display-2xs content-base-primary">
              {t("actions.uploadCertificate") || "Загрузка медицинской справки"}
            </h4>
          </div>
