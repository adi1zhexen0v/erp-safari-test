...
import type { GoodsSupplyContract, GoodsSupplyPreviewData } from "../types";
import {
...
interface Props {
  document: GoodsSupplyContract;
}

export default function GoodsSupplyCard({ document }: Props) {
  ...
  const [submitForSigning, { isLoading: isSubmitting }] = useSubmitGoodsSupplyForSigningMutation();
  const [downloadPreview, { isLoading: isDownloadingPreview }] = useDownloadGoodsSupplyPreviewMutation();
  const [deleteGoodsSupply, { isLoading: isDeleting }] = useDeleteGoodsSupplyMutation();
  const { data: documentData, isLoading: isLoadingDocumentData } = useGetGoodsSupplyQuery(document.id, {
  ...
  async function handleDownloadDraft() {
    try {
      ...
      downloadBlob(blob, `Договор поставки товара №${document.id}.docx`);
    } catch (err: unknown) {
      handleApiError(err, "messages.downloadFailed");
    }
  };

  function convertToPreviewData(doc: GoodsSupplyContract): GoodsSupplyPreviewData {
  ...
  async function handleConfirmDelete() {
    try {
      await deleteGoodsSupply(document.id).unwrap();
    ...
    <>
      ...
      {isPreviewOpen && (
        <>
          {isLoadingDocumentData || !documentData ? (
            <GoodsSupplyPreviewModalSkeleton onClose={handleClosePreview} />
          ) : (
            <GoodsSupplyPreviewModal
      ...
      {isEditOpen && (
        <ModalForm icon={DocumentText1} onClose={handleCloseEdit} allowCloseInOverlay={false}>
          <GoodsSupplyForm editId={document.id} onSuccess={handleEditSuccess} />
        </ModalForm>
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-body-bold-lg content-base-primary">
              {tTemplates("templates.supplyContract.title")} №{document.id}
            </p>
          </div>

          <div className="flex flex-col gap-3 border-t border-b surface-base-stroke py-3">
            <div className="flex flex-col gap-1.5">
              <span className="text-body-bold-xs content-base-secondary">{t("cards.supplier")}</span>
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-body-bold-xs content-base-secondary">{t("cards.product")}</span>
              <div className="flex items-center gap-1.5">
                <span className="content-action-brand">
                  <Box1 size={16} color="currentColor" />
                </span>
                <p className="text-body-regular-sm content-base-primary">{document.product_name}</p>
              </div>
            </div>

            <div className="flex flex-col gap-1.5">
              <span className="text-body-bold-xs content-base-secondary">{t("cards.manufacturer")}</span>
              <div className="flex items-center gap-1.5">
                <span className="content-action-brand">
                  <Buildings size={16} color="currentColor" />
                </span>
                <p className="text-body-regular-sm content-base-primary">{document.product_manufacturer}</p>
              </div>
            </div>

            <div className="flex flex-col gap-1.5">
              <span className="text-body-bold-xs content-base-secondary">{t("cards.totalAmount")}</span>
              <div className="flex items-center gap-1.5">
                <span className="content-action-brand">
                  <TengeCircleIcon size={16} />
                </span>
                <p className="text-body-regular-sm content-base-primary">
                  {formatPrice(document.product_unit_price)} ₸ x {document.product_quantity}{" "}
                  {t("cards.pieces").slice(0, -1)} ={" "}
                  {formatPrice(
                    document.total_amount ||
                      (Number(document.product_unit_price) * (document.product_quantity || 0)).toString(),
                  )}{" "}
                  ₸
                </p>
              </div>
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-body-bold-xs content-base-secondary">{t("cards.deliveryAddress")}</span>
              <div className="flex items-center gap-1.5">
                <span className="content-action-brand">
                  <Location size={16} color="currentColor" />
                </span>
                <p className="text-body-regular-sm content-base-primary">{document.delivery_address}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5 border-b surface-base-stroke pb-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-body-regular-sm content-base-primary">{formatDateForDisplay(document.created_at)}</span>
            </div>
          </div>
        </div>
