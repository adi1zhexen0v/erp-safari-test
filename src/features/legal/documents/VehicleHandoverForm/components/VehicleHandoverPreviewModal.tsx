...
import type { VehicleRentContract } from "../../VehicleRentForm/types";
import {
...
import type { VehicleHandoverPreviewData } from "../types";
...
export default function VehicleHandoverPreviewModal({ id, onClose, hasBackground = false, readOnly = false }: Props) {
  ...
  const { data: handoverData, isLoading: isLoadingHandover } = useGetVehicleHandoverQuery(id);
  ...
  const [submitForSigning, { isLoading: isSubmitting }] = useSubmitVehicleHandoverForSigningMutation();
  const [downloadPreview, { isLoading: isDownloadingPreview }] = useDownloadVehicleHandoverPreviewMutation();
  const [deleteHandover, { isLoading: isDeleting }] = useDeleteVehicleHandoverMutation();
  ...
  const parentContract: VehicleRentContract | undefined = handoverData?.parent_contract
    ? allDocuments?.vehicle_rentals.find((doc: VehicleRentContract) => doc.id === handoverData.parent_contract)
    : undefined;

  const previewData: VehicleHandoverPreviewData | null = handoverData
  ...
  const contractDateFormatted = parentContract.contract_date
    ? formatDateForContract(new Date(parentContract.contract_date), locale)
        ...
        <div className="flex-1 overflow-auto page-scroll pr-5 pt-5">
          <div className="flex flex-col text-body-regular-md content-base-primary">
            ...
            <div className="text-center mb-6">
              <p className="text-sm mb-2">Приложение №1 к Договору аренды транспортного средства</p>
              <p className="text-sm mb-4">№ {handoverData.id}</p>
              ...
              <div className="flex justify-between text-sm mt-4">
                ...
                <strong>{previewData.act_date}</strong>
              </div>
            </div>
            ...
            <div className="mb-6 text-justify">
              <p className="mb-4">
                В целях исполнения Договора аренды транспортного средства №{parentContract.id} от{" "}
              ...
              <p className="mb-2">
                ...
                , в лице <strong>{parentContract.commercial_org?.representative || "_____________"}</strong>,
                действующего на основании <strong>{parentContract.commercial_org?.basis || "_____________"}</strong>,
                именуемый в дальнейшем «Арендодатель», с одной стороны, и{" "}
                <strong>{userData?.organization || "_____________"}</strong> в лице директора{" "}
                <strong>{userData?.full_name || "_____________"}</strong>, действующего на основании Устава, именуемый в
                дальнейшем «Арендатор», с другой стороны, составили настоящий акт о нижеследующем:
              </p>

              <p className="mb-4">
                Арендодатель передал, а Арендатор принял во временное владение и пользование транспортное средство со
                следующими характеристиками:
              </p>

              <ul className="list-none ml-6 mb-4">
                <li className="mb-2">
                  Автомобиль марки <strong>{parentContract.car_brand || "_____________"}</strong>
                </li>
                <li className="mb-2">
                  <strong>{parentContract.car_year || "_____________"}</strong> года выпуска
                </li>
                <li className="mb-2">
                  VIN/кузов/шасси <strong>{parentContract.car_vin || "_____________"}</strong>
                </li>
                <li className="mb-2">
                  регистрационный номер <strong>{parentContract.car_plate || "_____________"}</strong>
                </li>
                <li className="mb-2">
                  цвет <strong>{parentContract.car_color || "_____________"}</strong>
                </li>
              </ul>

              <p className="mb-4">
                Стороны подтверждают, что на момент приема-передачи транспортное средство находится в технически
